 ...
 * `YYYY MM DD DDD HH hh h mm ss SSS A a ddd dddd MMM MMMM` plus `[literal]`.
 */
...
const TOKEN_REGEX = /YYYY|MMMM|MMM|MM|DDD|DD|dddd|ddd|HH|hh|h|mm|ss|SSS|A|a|\[[^\]]*\]/g;
// Patterns astrotime can format/parse natively: only tokens it defines
// (YYYY MM DD DDD HH mm ss SSS), bracket literals and non-letter separators.
const ASTROTIME_SAFE_PATTERN = /^(?:YYYY|MM(?!M)|DDD|DD|HH|mm|ss|SSS|\[[^\]]*\]|[^A-Za-z[\]])*$/;
...
export function parseUtc(text, patterns) {
  for (const pattern of [].concat(patterns)) {
    if (ASTROTIME_SAFE_PATTERN.test(pattern) && pattern.includes('YYYY')) {
